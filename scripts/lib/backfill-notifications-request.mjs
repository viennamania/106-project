...
function normalizeRequestPayload(overrides = {}) {
  ...
  const email =
    typeof overrides.email === "string"
      ...
      : process.env.NOTIFICATIONS_BACKFILL_EMAIL?.trim() ??
        process.env.RECONCILE_EMAIL?.trim();
  const rawLimit =
    typeof overrides.limit === "number"
      ? String(overrides.limit)
      : process.env.NOTIFICATIONS_BACKFILL_LIMIT?.trim() ??
...
export async function runBackfillNotificationsRequest(overrides) {
  ...
  const response = await fetch(
    new URL("/api/internal/backfill-notifications", baseUrl),
    {
  ...
  if (!response.ok) {
    throw new Error(
      `Notification backfill request failed (${response.status}): ${
