...
1. **셀러 UI** — 옷 업로드 + 스타 선택 + 결과 갤러리 (`src/app/[lang]/fanletter/lookbook` 신설 예정)
2. **결제/GTM** — 셀러는 USDT 안 씀 → 원화/포인트 룩북 과금 모델 (현 최대 리스크)
3. **TikTok 유통 연동** — Codex의 AI 스타↔TikTok 연결 완료 후, 룩북 자동 게시
4. **품질 튜닝** — 원단/프린트/로고 보존 한계 시 해상도 상향·레퍼런스 다각도·후처리
5. **한국형 배경 프리셋** — sceneBrief 템플릿화 (성수/한남/스튜디오 등)
