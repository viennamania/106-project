...
## Seller lookbook worker (async generation)

Heavy seller-lookbook generation (multi-shot batches, video) should not run on a
user-facing Vercel function (300s ceiling, cost). The seller API enqueues a job
and a Railway worker drains the queue, running generation off-request.

Flow:

1. `POST /api/seller/lookbook/job` — charges credits up front, creates a
   `queued` job (`sellerLookbookJobs`), returns a `jobId`.
2. The Railway worker (`railway/lookbook-worker`) loops, calling the internal
   process route which claims one job and runs the generation.
3. Seller polls `GET /api/seller/lookbook/job?jobId=...` until `done`/`failed`.
   Failed jobs auto-refund the charged credits.

Endpoint added for the worker:

- `POST /api/internal/seller-lookbook/process`
  - Protected by `Authorization: Bearer <SELLER_JOB_WORKER_TOKEN>`.
  - Claims the oldest queued job, runs it (image set or video), records
    provenance + star royalty, completes or fails (with refund).

Worker service (`railway/lookbook-worker`, continuous — not cron) env:

- `LOOKBOOK_PROCESS_URL` — full URL of the process route. Prefer the Railway API
  internal URL, e.g. `http://api.railway.internal:8080/api/internal/seller-lookbook/process`.
- `SELLER_JOB_WORKER_TOKEN` — shared secret; also set on the API/Vercel env so
  the route can verify it.
- `LOOKBOOK_WORKER_INTERVAL_MS` — idle poll interval when the queue is empty (default `5000`).

Notes:

- Run the process route on the Railway API service (no function timeout), not
  only on Vercel, so long video jobs are not capped.
- Video also needs a FAL_KEY with reference-to-video access
  (`FAL_LOOKBOOK_VIDEO_MODEL`); the queue handles execution, not model access.
