...
export const FANLETTER_CHARACTER_GROWTH_MAX_LEVEL = 10;
export const FANLETTER_CHARACTER_GROWTH_LEVEL_XP_FLOORS = [
  0, 140, 360, 680, 1120, 1680, 2380, 3240, 4280, 5520,
] as const;

export type FanletterCharacterGrowthLevelState = {
  level: number;
  maxLevel: number;
  nextLevelXp: number | null;
  progressPercent: number;
...
type GrowthCopyUnlock = Omit<FanletterCharacterGrowthUnlock, "unlocked"> & {
  requirement: (metrics: FanletterCharacterGrowthMetrics, level: number) => boolean;
};

type GrowthCopyMission = Omit<
...
export type FanletterCharacterGrowthCopy = {
  avatarUnlocks: GrowthCopyUnlock[];
  contentSkills: GrowthCopyUnlock[];
  missions: GrowthCopyMission[];
...
export function getFanletterCharacterGrowthCopy(
  locale: Locale,
): FanletterCharacterGrowthCopy {
  if (locale === "ko") {
    return {
      avatarUnlocks: [
        {
      ...
      summary: (metrics, level) =>
        metrics.publishedVlogCount > 0
          ? `공개 브이로그 ${metrics.publishedVlogCount}개, 팬 반응 ${
              metrics.likeCount + metrics.commentCount + metrics.saveCount
            }개, 팬 요청 ${metrics.fanRequestTotalCount}개를 기반으로 Lv.${level}까지 성장했습니다.`
  ...
  return {
    ...
    summary: (metrics, level) =>
      metrics.publishedVlogCount > 0
        ? `Reached Lv.${level} from ${metrics.publishedVlogCount} public vlogs, ${
            metrics.likeCount + metrics.commentCount + metrics.saveCount
          } fan reactions, and ${metrics.fanRequestTotalCount} fan requests.`
...
export function calculateFanletterCharacterGrowthLevel(
  totalXp: number,
): FanletterCharacterGrowthLevelState {
  let level = 1;

  for (
    let index = 0;
    index < FANLETTER_CHARACTER_GROWTH_LEVEL_XP_FLOORS.length;
    index += 1
  ) {
    if (totalXp >= FANLETTER_CHARACTER_GROWTH_LEVEL_XP_FLOORS[index]) {
      level = index + 1;
    }
  }

  const maxLevel = FANLETTER_CHARACTER_GROWTH_MAX_LEVEL;
  const currentLevelFloor = FANLETTER_CHARACTER_GROWTH_LEVEL_XP_FLOORS[level - 1] ?? 0;
  const nextLevelXp =
    level >= maxLevel
      ? null
      : FANLETTER_CHARACTER_GROWTH_LEVEL_XP_FLOORS[level] ?? null;
  const xpInCurrentLevel = Math.max(0, totalXp - currentLevelFloor);
  const xpToNextLevel =
    nextLevelXp === null ? null : Math.max(0, nextLevelXp - totalXp);
  const progressPercent =
    nextLevelXp === null
      ? 100
      : Math.round(
          (xpInCurrentLevel / Math.max(1, nextLevelXp - currentLevelFloor)) *
            100,
        );
...
export function calculateFanletterCharacterGrowthTotalXp({
  characterReady,
  metrics,
}: {
...
export function buildFanletterCharacterGrowthMissions({
  characterReady,
  copy,
  metrics,
}: {
  characterReady: boolean;
  copy: FanletterCharacterGrowthCopy;
  metrics: FanletterCharacterGrowthMetrics;
}): FanletterCharacterGrowthMission[] {
...
export function buildFanletterCharacterGrowthRecord({
  characterReady,
  locale,
  metrics,
}: {
  ...
  const totalXp = calculateFanletterCharacterGrowthTotalXp({
    characterReady,
    metrics,
  });
  const levelState = calculateFanletterCharacterGrowthLevel(totalXp);
  const copy = getFanletterCharacterGrowthCopy(locale);
  ...
  return {
    ...
    metrics,
    missions: buildFanletterCharacterGrowthMissions({
      characterReady,
      copy,
      metrics,
    }),
