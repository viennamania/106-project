...
export function getFanletterNsfwCopy(locale: Locale) {
  return locale === "ko"
    ? {
        badge: "NSFW",
        disabledBody:
          "NSFW 콘텐츠는 별도 opt-in 후 피드와 팬 전용 영역에서 표시됩니다.",
        disabledCta: "NSFW 보기 켜기",
        disabledTitle: "NSFW 숨김",
        enabledBody:
          "NSFW 콘텐츠를 피드, 캐릭터 채널, 팬 전용 영역에서 함께 표시합니다.",
        enabledCta: "NSFW 보기 끄기",
        enabledTitle: "NSFW 표시 중",
        hiddenCount: (count: string) => `숨겨진 NSFW 콘텐츠 ${count}개`,
        studioBody:
          "영상 콘텐츠에 적용할 수 있습니다. 기본 피드에서는 숨겨지고, opt-in한 팬에게 NSFW 배지와 함께 강조됩니다.",
        studioLabel: "NSFW로 표시",
        studioTitle: "NSFW opt-in 노출",
      }
    : {
        badge: "NSFW",
        disabledBody:
          "NSFW content appears only after a separate opt-in.",
        disabledCta: "Show NSFW",
        disabledTitle: "NSFW hidden",
        enabledBody:
          "NSFW content is shown across feeds, character channels, and fan-only areas.",
        enabledCta: "Hide NSFW",
        enabledTitle: "NSFW visible",
        hiddenCount: (count: string) => `${count} NSFW posts hidden`,
        studioBody:
          "Available for video content. It stays hidden in the default feed and is highlighted with an NSFW badge for opted-in fans.",
        studioLabel: "Mark as NSFW",
