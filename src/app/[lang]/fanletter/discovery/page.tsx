...
type FanletterDiscoverySearchParams = {
  page?: string | string[];
  q?: string | string[];
  ref?: string | string[];
  sort?: string | string[];
...
function readDiscoveryPage(rawValue?: string | string[]) {
  const parsed = Number(readFirstSearchParam(rawValue));

  return Number.isFinite(parsed) ? Math.max(1, Math.floor(parsed)) : 1;
}

function getDiscoveryMeta(locale: Locale) {
  if (locale === "ko") {
    return {
      description:
        "AIAVpark에서 브이로그와 팬 반응으로 성장 중인 AI 스타를 발견하고 Founder 참여로 이어가세요.",
...
export async function generateMetadata({
  ...
  searchParams: Promise<FanletterDiscoverySearchParams>;
}): Promise<Metadata> {
  const { lang } = await params;
  const query = await searchParams;
  const locale = hasLocale(lang) ? lang : defaultLocale;
  const referralCode = readFanletterReferralCode(query.ref);
  const meta = getDiscoveryMeta(locale);
  ...
  const ogImagePath = buildFanletterOgImagePath({
    ...
    title: meta.title,
    variant: "feed",
    version: "fanletter-discovery-v1",
  });
  const ogImage = {
...
export default async function LocalizedFanletterDiscoveryPage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string }>;
  ...
  const data = await getFanletterCharacterDirectoryPageData(
    ...
    {
      includeNsfw,
      page: readDiscoveryPage(query.page),
      query: readFirstSearchParam(query.q),
      sort: readDiscoverySort(query.sort),
    },
  );
  ...
  return (
    <FanletterCharacterDirectoryPage
      ...
      routePath={`/${locale}/fanletter/discovery`}
    />
  );
}
