...
type FanletterNewsBnbWalletSearchParams = {
  ref?: string | string[];
...
export async function generateMetadata({
  ...
  searchParams: Promise<FanletterNewsBnbWalletSearchParams>;
}): Promise<Metadata> {
  ...
  const returnToHref = getSafeFanletterReturnTo({
    fallbackPath: `/${locale}/fanletter/news/wallet/manage`,
    locale,
    referralCode,
  ...
  const ogImagePath = buildFanletterOgImagePath({
    ...
    title,
    variant: "start",
    version: "fanletter-news-wallet-bnb-v1",
  });
  const ogImage = {
...
export default async function LocalizedFanletterNewsBnbWalletPage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string }>;
