import { reconcileCompletedMemberNetworks } from "@/lib/member-service";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await reconcileCompletedMemberNetworks({
      email: body.email,
      limit: body.limit,
    });
    ...
    return jsonError(
      error instanceof Error
        ? error.message
        : "Failed to reconcile completed member networks.",
      500,
    );
  }
}
