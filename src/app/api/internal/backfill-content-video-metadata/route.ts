import {
  backfillContentVideoMetadata,
  type ContentVideoMetadataBackfillInput,
} from "@/lib/content-video-metadata-backfill-service";
...
async function readBackfillRequestBody(
  request: Request,
): Promise<ContentVideoMetadataBackfillInput> {
...
export async function POST(request: Request) {
  ...
  let body: ContentVideoMetadataBackfillInput;
  ...
  try {
    const result = await backfillContentVideoMetadata(body);
    ...
    return jsonError(
      error instanceof Error
        ? error.message
        : "Failed to backfill content video metadata.",
      500,
    );
  }
}
