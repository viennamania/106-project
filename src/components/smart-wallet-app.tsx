...
function getActivationSeparationCopy(locale: Locale) {
  if (locale === "ko") {
    return {
      ...
      coreStatusDescription:
        "내 코드 가입 회원, 교환 가능 포인트, 지갑, AI 스타 프로필을 각각 전용 화면에서 확인하세요.",
      coreStatusTitle: "가입 완료",
      growthDescription:
        "추천 가입으로 쌓인 포인트와 보상 내역은 포인트 내역에서 확인합니다.",
  ...
  return {
    ...
    coreStatusDescription:
      "Review joined members, redeemable points, wallet, and AI Star profile in dedicated screens.",
...
function getActivationHubCopy(locale: Locale) {
  if (locale === "ko") {
    return {
      ...
      feedDescription: "전체 공개 콘텐츠와 작성자를 피드에서 확인합니다.",
      feedLabel: "콘텐츠 피드",
      feedMetric: "피드 보기",
      hubLabel: "1066FRIEND+",
      lockedMenuCaption: "이메일 인증 후 이용할 수 있습니다.",
      menuLockedDescription: "이메일 인증 후 추천 코드와 지갑 화면을 사용할 수 있습니다.",
      menuLockedTitle: "먼저 이메일로 시작하세요",
      menuOpenAfterLogin: "이메일 인증 후 이용 가능",
      menuPendingDescription: "10 USDT 확인 후 추천, 포인트, 지갑 화면이 열립니다.",
      menuPendingTitle: "서비스 이용료 확인이 필요합니다",
      menuReadyTitle: "다음으로 할 일",
      ...
      nextActionReasonDefault:
        "내 코드 가입 회원 화면에서 가입자와 다음 보상 흐름을 확인할 수 있습니다.",
      ...
      studioDescription: "가입 후 생성된 AI 스타 이름과 프로필 이미지를 관리합니다.",
      studioLabel: "AI 스타 관리",
      studioProfileMetric: "프로필 설정",
      studioReadyMetric: "프로필 관리",
      title: "1066FRIEND+ 시작하기",
      walletDescription: "USDT와 BNB 잔고, 받을 주소, 전송 상태를 확인합니다.",
      walletLabel: "내 지갑",
    };
  }
  ...
  return {
    ...
    feedDescription: "Review all public content and reporter identity in the feed.",
    feedLabel: "Content feed",
    feedMetric: "Open feed",
    hubLabel: "1066FRIEND+",
    lockedMenuCaption: "Available after email verification.",
    menuLockedDescription: "Verify email to use the referral code and wallet screens.",
    menuLockedTitle: "Start with email first",
    menuOpenAfterLogin: "Available after email",
    menuPendingDescription: "Referral, points, and wallet screens open after the 10 USDT check.",
    menuPendingTitle: "Service fee check needed",
    menuReadyTitle: "What to do next",
    ...
    networkLabel: "Members from my code",
    ...
    nextActionReasonDefault:
      "Use members from my code to review signups and the next reward flow.",
    ...
    studioDescription: "Manage the AI Star name and profile image created after signup.",
    studioLabel: "Manage AI Star",
    studioProfileMetric: "Set profile",
    studioReadyMetric: "Manage profile",
    title: "Start 1066FRIEND+",
    walletDescription: "Check USDT, BNB, receive address, and transfer status.",
...
export function SmartWalletApp({
  ...
  return (
    <div className="friend-service-surface relative isolate">
      ...
      <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-3 px-4 py-4 pb-32 pt-[calc(env(safe-area-inset-top)+5.9rem)] sm:gap-4 sm:px-6 sm:py-6 sm:pb-32 sm:pt-6 lg:px-8 lg:pb-8">
...
function ActivationServiceHub({
  ...
  const statusItems = [
    ...
    ...(isSignupCompleted
      ? [
          {
            actionLabel: locale === "ko" ? "프로필 관리" : "Manage profile",
  ...
  const quickLabels =
    locale === "ko"
      ? {
          copied: "복사 완료",
          current: "현재 상태",
          manageNetwork: "내 코드 가입 회원 보기",
          manageRewards: "리워드 / 포인트 보기",
          points: "누적 적립 포인트",
          pointsCaption: "교환 가능 포인트는 리워드 화면에서 확인",
          ...
          snapshot: "가입 요약",
          updated: "최근 확인",
          updatedCaption: "최근 데이터 확인 시각",
        }
      : {
          copied: "Copied",
          current: "Current status",
          manageNetwork: "View members from my code",
          manageRewards: "View rewards / points",
          points: "Lifetime earned points",
          pointsCaption: "Redeemable points are shown on the rewards screen",
          referralCode: "Referral code",
          ...
          refresh: "Refresh",
          signups: "Joined from my code",
          signupsCaption: "Members joined with your code",
          snapshot: "Signup snapshot",
          updated: "Last checked",
          updatedCaption: "Latest data check time",
        };
