          ...
          <table className="w-full table-fixed border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50/90">
                <th
                  className="w-[22%] border-b border-slate-200 px-1.5 py-3 text-left text-[0.64rem] font-semibold uppercase tracking-[0.14em] text-slate-500 md:w-[6.5rem] md:px-4 md:text-[0.68rem] md:tracking-[0.18em]"
                  scope="col"
                >
                  {levelLabel}
                </th>
                <th
                  className="w-[50%] border-b border-slate-200 px-1.5 py-3 text-left text-[0.64rem] font-semibold uppercase tracking-[0.14em] text-slate-500 md:w-[15rem] md:px-4 md:text-[0.68rem] md:tracking-[0.18em]"
                  scope="col"
                >
                  <div className="flex flex-col gap-1">
                    <span>{sourceMemberLabel}</span>
                    <span className="text-[0.58rem] leading-4 text-slate-400 md:text-[0.62rem]">
                      {awardedAtLabel}
                    </span>
                  </div>
                </th>
                <th
                  className="w-[28%] border-b border-slate-200 px-1.5 py-3 text-right text-[0.64rem] font-semibold uppercase tracking-[0.14em] text-slate-500 md:w-[7rem] md:px-4 md:text-[0.68rem] md:tracking-[0.18em]"
                  scope="col"
                >
                  {pointsLabel}
                </th>
                  ...
                  <tr className="align-top" key={`${reward.sourceMemberEmail}:${reward.level}:${reward.awardedAt}`}>
                    ...
                    <td className={cn("px-1.5 py-3.5 md:px-4", rowBorderClass)}>
                      <div className="space-y-1.5">
                        <p className="break-all text-[0.76rem] leading-5 font-medium text-slate-900 md:text-sm md:leading-6">
                          {reward.sourceMemberEmail}
                        </p>
                        <p className="text-[0.62rem] leading-4 text-slate-500 md:text-xs md:leading-5">
                          {formatDateTime(reward.awardedAt, locale)}
                        </p>
                      </div>
                    </td>
                    <td className={cn("px-1.5 py-3.5 text-right md:px-4", rowBorderClass)}>
                      <span className="ml-auto inline-flex whitespace-nowrap rounded-full border border-emerald-200 bg-emerald-50 px-1.5 py-1 text-[0.66rem] font-semibold text-emerald-900 tabular-nums md:px-3 md:text-sm">
